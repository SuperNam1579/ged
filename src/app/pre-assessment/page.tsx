...
export default function PreAssessmentPage() {
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-sm px-4">
          ...
          <p className="text-gray-500 text-sm">
            Our AI is reviewing your answers and setting up your personalized study plan. This will just take a moment.
          </p>
        </div>
      </div>
    );
  ...
  const globalQuestionNumber =
    assessments.slice(0, currentAssessmentIdx).reduce((sum, a) => sum + a.questions.length, 0) +
    currentQuestionIdx +
    1;
  ...
  const handleNext = async () => {
    ...
    const isLastQuestion = currentQuestionIdx === currentAssessment.questions.length - 1;
      ...
      <header className="bg-white border-b border-gray-100 px-6 py-4">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-lg font-bold text-gray-900">Pre-Assessment</span>
          </div>
          <div className="flex items-center gap-3">
            <span
              className={cn(
                "px-2.5 py-1 rounded-full text-xs font-semibold",
                SUBJECT_COLORS[currentAssessment.subjectCode] ?? "bg-gray-100 text-gray-700"
              )}
            >
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-3">
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs text-gray-500 font-medium">Question {globalQuestionNumber} of {totalQuestions}</span>
            <span className="text-xs text-gray-500 font-medium">{Math.round((answeredCount / totalQuestions) * 100)}%</span>
          </div>
          <ProgressBar value={(globalQuestionNumber / totalQuestions) * 100} showPercent={false} variant="blue" size="sm" />
        </div>
      </div>
            ...
            <div
              ...
              className={cn(
                ...
                i === currentAssessmentIdx
                  ? SUBJECT_COLORS[a.subjectCode] ?? "bg-blue-100 text-blue-700"
                  : i < currentAssessmentIdx
                  ? "bg-green-100 text-green-700"
                  : "bg-gray-100 text-gray-400"
              )}
            >
              {i < currentAssessmentIdx ? "✓ " : ""}{a.subjectCode}
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="w-full max-w-2xl">
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 mb-6">
            <p className="text-xs text-gray-400 uppercase tracking-wide font-medium mb-4">
              Question {currentQuestionIdx + 1} of {currentAssessment.questions.length}
            </p>
          ...
          <div className="space-y-3 mb-8">
            {currentQuestion.options.map((option, i) => (
              ...
              >
                ...
                <span className={cn("text-sm font-medium", selectedOption === option.id ? "text-blue-900" : "text-gray-700")}>
                  {option.text}
                </span>
              </button>
            ))}
